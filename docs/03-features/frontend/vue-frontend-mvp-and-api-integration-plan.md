...
- space detail view with embedded shared chat
...
The browser-facing frontend now uses the implemented authentication model from the current feature set:
...
- the frontend bootstraps CSRF through `GET /api/auth/csrf` before the first unsafe request when needed
- unsafe browser requests mirror the readable `XSRF-TOKEN` cookie into the `X-XSRF-TOKEN` header
- the frontend does not persist the auth JWT in `localStorage`
- `POST /api/auth/logout` clears the session cookie server-side

Implementation note:
- route/session restoration still relies on `GET /api/auth/me`
- the frontend does not parse or store the JWT because authentication is resolved server-side from the auth cookie
- MFA-enabled login attempts transition to an `MFA_REQUIRED` UI state and only receive the authenticated session cookie after `POST /api/auth/mfa/verify` succeeds
...
- read and post in the shared chat for spaces they currently belong to, unless the space is archived
...
### Lecturer
- login
- view and post in shared chat for spaces they own
- create assignment
...
5. space detail screen with embedded polling-based space chat
