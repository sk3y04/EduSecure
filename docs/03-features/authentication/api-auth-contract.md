...
### Implementation note
- password verification is delegated through Spring Security `AuthenticationManager`/`DaoAuthenticationProvider`
- `CustomUserDetailsService` loads the user record and `BCryptPasswordEncoder` verifies the submitted password hash match
- after successful password authentication, `AuthService` reloads the normalized user entity to decide between immediate JWT issuance and the `MFA_REQUIRED` branch
...
### Implementation note
Authentication for `/api/auth/me` is established before controller logic runs:
- `JwtAuthenticationFilter` resolves the JWT from the auth cookie first
- if no auth cookie is present, it can fall back to `Authorization: Bearer <jwt>`
- the filter validates the signed JWT, loads `UserDetails`, and populates the Spring Security context
- `AuthController` then calls `AuthService.currentUser(authentication.getName())`
...
It does **not** cover every security-layer rejection. Requests denied by Spring Security before controller/service handling
such as missing authentication on protected endpoints, CSRF failures, or authorization failures may still surface as plain
`401 Unauthorized` or `403 Forbidden` responses rather than the structured JSON envelope.
