...
- `backend/src/main/java/edusecure/edusecure/controller/system/SystemController.java`
...
- `backend/src/main/java/edusecure/edusecure/service/auth/TotpProvider.java`
...
- `backend/src/main/java/edusecure/edusecure/service/auth/AuthApiException.java`
...
| `/api/auth/csrf` | GET | Implemented | Public CSRF bootstrap endpoint for browser clients before unsafe requests |
...
- `/api/auth/me` authentication is established by `JwtAuthenticationFilter` before controller logic runs
- the filter resolves JWTs from the auth cookie first and only then falls back to `Authorization: Bearer <jwt>` for compatibility
- password verification is delegated through Spring Security `AuthenticationManager` and `DaoAuthenticationProvider` using `BCryptPasswordEncoder`
...
- security-layer failures that happen before controller/service code, such as missing authentication on protected endpoints or CSRF rejection, may still return plain `401`/`403` responses
...
- `GET /api/auth/csrf` issues the readable CSRF cookie used by unsafe browser requests
...
- register and login reject unsafe requests that omit the required CSRF token
