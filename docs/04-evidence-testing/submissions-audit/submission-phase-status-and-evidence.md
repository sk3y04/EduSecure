...
The backend now includes a minimal implementation aligned with the submissions-and-audit design set for:
- `Assignment`
- `Submission`
...
- uses a multipart browser upload with one required `file` part
- currently supports UTF-8 `text/plain` and validated `application/pdf`
- enforces a bounded upload size of `5MB`
...
- student can upload a text or validated PDF submission file to an assignment
...
- unsupported upload types are rejected within the current bounded scope
