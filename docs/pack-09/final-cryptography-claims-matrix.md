...
| `TLS 1.3` via Certbot/Let's Encrypt | TLS 1.3 via Certbot/Let's Encrypt is documented as the intended deployment-side transport control for secure client-server communication | Transport confidentiality and integrity in the secure design narrative; directly addresses the brief's Wi-Fi token-interception incident | `docs/pack-09/report-claims-audit-note.md`; deployment appendix/design notes | "EduSecure documents TLS 1.3 via Certbot/Let's Encrypt as the intended deployment-side control for protecting credentials and session tokens in transit." | Do not present HTTPS enforcement as repository-proven unless you include direct deployment evidence. |
...
- Secure transmission is addressed in the secure design/deployment narrative through TLS 1.3 via Certbot/Let's Encrypt, while the implemented symmetric-encryption evidence comes from the AES-GCM at-rest controls.
