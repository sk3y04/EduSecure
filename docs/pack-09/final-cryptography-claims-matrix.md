...
| AES-GCM secure transmission demo | Dedicated demo endpoints encrypt and decrypt message payloads using AES-GCM with explicit nonce handling | Demonstrated symmetric confidentiality and authenticated encryption | `docs/pack-08/aes-demo-phase-status-and-evidence.md`; `backend/src/test/java/edusecure/edusecure/AesDemoIntegrationTests.java` | "EduSecure includes a separate AES-GCM secure-transmission demonstration that shows nonce handling, encryption, decryption, and tamper detection." | Do not imply that all normal EduSecure API traffic is application-layer AES encrypted. |
| AES-GCM protection of submission content at rest | Submission content is encrypted before durable storage, while metadata stays available through a separate endpoint split | Confidentiality-at-rest for a real EduSecure business asset | `docs/pack-06/submission-content-protection-and-retrieval.md`; `SubmissionFlowIntegrationTests.java`; `SubmissionContentEncryptionServiceTests.java`; `SubmissionKeyProtectionServiceTests.java`; `FileSystemSubmissionContentStoreTests.java` | "EduSecure applies AES-GCM to protect stored submission content at rest while preserving a separate digest/signature workflow for integrity and authorship." | Do not claim enterprise KMS/HSM maturity or end-to-end encrypted submission transport. |
...
| `TLS 1.3` secure design narrative | Present in secure design rationale rather than directly repo-proven deployment evidence | Transport confidentiality and integrity in the system design discussion | `docs/pack-02/crypto-decision-matrix.md`; `docs/pack-02/cia-evaluation.md`; UML/report design notes | "TLS 1.3 is part of the secure system design narrative for client-server communication." | Do not state that the repository alone proves end-to-end deployment-time TLS hardening. |
...
- AES-GCM appears in two different roles:
  - a separate secure-transmission demo
  - real confidentiality-at-rest for stored submission content
