...
### 1.1 Optional MongoDB coexistence note

PostgreSQL remains the source of truth for users, roles, spaces, memberships, authentication, and audit logs.

The repository now also includes a MongoDB-backed chat path for per-space messages:
- it is enabled with `APP_CHAT_ENABLED=true`
- MongoDB is configured through `SPRING_DATA_MONGODB_URI` and `SPRING_DATA_MONGODB_DATABASE`
- plain `docker compose up` starts MongoDB alongside the rest of the stack

This MongoDB path is additive; it does not replace the PostgreSQL/Liquibase runtime baseline.
...
Example local workflow with shared space chat enabled:
